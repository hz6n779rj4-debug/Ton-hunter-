...
async function getDbTokens(status?: 'approved' | 'pending' | 'rejected' | 'pending_payment'): Promise<ListedToken[]> {
  if (!supabaseAdmin) return sampleTokens;
  let query = supabaseAdmin.from('tokens').select('*').order('promoted', { ascending: false }).order('votes_24h', { ascending: false }).order('listed_at', { ascending: false });
  if (status) query = query.eq('status', status);
  const { data, error } = await query;
  if (error || !data?.length) return status === 'approved' || !status ? sampleTokens : [];
...
async function enrichToken(token: ListedToken): Promise<ListedToken> {
  try {
    ...
    return {
      ...
      logo_url: token.logo_url || jettonJson?.metadata?.image,
      ...
      chart_url: resolved?.dex_url || token.chart_url,
    };
  } catch {
    return token;
  }
...
function isCurrentlyPromoted(token: ListedToken) {
  if (!token.promoted) return false;
  if (!token.feature_expires_at) return true;
  return new Date(token.feature_expires_at).getTime() > Date.now();
}

export async function getTokens(): Promise<ListedToken[]> {
  const tokens = await getDbTokens('approved');
  return Promise.all(tokens.map((token) => enrichToken(token)));
}

export async function getAdminTokens(): Promise<ListedToken[]> {
...
export async function getHomepageData() {
  const tokens = await getTokens();
  const promoted = tokens.filter((token) => isCurrentlyPromoted(token)).slice(0, 3);
  ...
  const stats = {
    ...
    promotedCount: tokens.filter((token) => isCurrentlyPromoted(token)).length,
