...
import { ExternalLink, Globe, MessageCircle, Twitter, Vote, Sparkles, BadgeCheck } from 'lucide-react';
...
export default async function TokenPage({
  ...
  searchParams?: Promise<{ vote?: string; next?: string }>;
}) {
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="card p-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
            <Image src={token.logo_url} alt={token.name} width={120} height={120} className="h-24 w-24 rounded-3xl object-cover" unoptimized />
...
function Social({ href, label, icon }: { href: string; label: string; icon: React.ReactNode }) {
  return <Link href={href} target="_blank" className="inline-flex items-center gap-2 rounded-full border border-stroke px-4 py-2 text-sm text-slate-200 hover:border-cyan-400/40">{icon}{label}</Link>;
}

function Stat({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-stroke/70 bg-slate-950/20 p-4"><div className="text-xs uppercase tracking-wide text-slate-500">{label}</div><div className="mt-2 text-lg font-semibold">{value}</div></div>;
}

function Line({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-stroke/60 bg-slate-950/20 p-4"><div className="text-xs uppercase tracking-[0.18em] text-slate-500">{label}</div><div className="mt-2 break-all text-sm text-slate-200">{value}</div></div>;
}

function Notice({ tone, text }: { tone: 'success' | 'warning' | 'error'; text: string }) {
  const classes = tone === 'success' ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300' : tone === 'warning' ? 'border-amber-500/30 bg-amber-500/10 text-amber-200' : 'border-rose-500/30 bg-rose-500/10 text-rose-300';
