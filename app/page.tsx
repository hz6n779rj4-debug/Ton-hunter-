...
export default async function HomePage() {
  ...
  return (
    <div>
      ...
      <section id="top-gainers" className="container-main py-8"><SectionHeading title="Top Gainers" subtitle="Sorted by strongest positive change, with volume supporting the rank." action={<Link href="/explore" className="inline-flex items-center gap-1 text-sm text-cyan-300 hover:text-cyan-200">Explore <ArrowRight className="h-4 w-4" /></Link>} /><div className="grid gap-4 lg:grid-cols-3">{topGainers.map((token, index) => <TokenCard key={token.id} token={token} rank={index + 1} />)}</div></section>
