      ...
      <div className="card mx-auto max-w-3xl p-6">
        ...
        <form action="/api/claim-project" method="post" className="mt-6 grid gap-4">
          <label className="grid gap-2 text-sm">
            ...
            <input name="token_address" required className="rounded-2xl border border-stroke bg-slate-950/30 px-4 py-3 outline-none focus:border-cyan-400/50" placeholder="EQ..." />
          </label>
          <label className="grid gap-2 text-sm">
            <span className="text-slate-300">Telegram username</span>
            <input name="username" className="rounded-2xl border border-stroke bg-slate-950/30 px-4 py-3 outline-none focus:border-cyan-400/50" placeholder="@yourhandle" />
          </label>
          <label className="grid gap-2 text-sm">
            <span className="text-slate-300">Proof</span>
            <textarea name="proof" rows={5} required className="rounded-2xl border border-stroke bg-slate-950/30 px-4 py-3 outline-none focus:border-cyan-400/50" placeholder="Share your Telegram/X/website proof showing you control the project." />
          </label>
          <button className="rounded-full bg-white px-5 py-3 font-medium text-slate-950">Send claim request</button>
        </form>
      </div>
    </section>
  );
