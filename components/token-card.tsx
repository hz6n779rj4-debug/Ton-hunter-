...
import { ArrowUpRight, TrendingUp, Vote } from 'lucide-react';
    ...
    <div className="card p-4 transition duration-200 hover:-translate-y-1 hover:border-cyan-400/35 hover:shadow-glow">
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400">
          <span className="inline-flex items-center gap-1"><Vote className="h-3.5 w-3.5" />{formatCompact(token.votes_24h)} 24h votes</span>
          <span>{formatCompact(token.votes_all_time)} all-time</span>
        </div>
